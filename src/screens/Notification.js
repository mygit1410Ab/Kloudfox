...
const Notification = () => {
  const accessToken = useSelector((state) => state.token.access); // 👈 Access token from Redux
  let allUnread = [];
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          styles.itemCard,
          { shadowColor: item?.severity === "green" ? "#22C55F" : "red" },
        ]}
      >
        <View
          style={{
      ...
      <View style={styles.hearderCard}>
        <TouchableOpacity
          ...
          style={[
            ...
            { borderColor: borderColor, borderWidth: recent ? 1 : 0 },
          ]}
        >
          <Text
            style={[
  ...
  const markasReadHandler = async (selectedItem) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${baseUrl}${endPoints.postNotifications}`,
        data,
        {
          headers: {
      ...
      >
        <View style={styles.modalStyle}>
          <View style={styles.modalContainCard}>
            ...
            >
              <Image style={{ height: 40, width: 40 }} source={images.cross} />
            </TouchableOpacity>
            ...
            >
              <Text style={[FontsStyle.smallButton_text]}>Mark as Read</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
