...
import { requestUserPermission } from "../PushNotification/PushNotification";
      ...
      if (Platform.OS === "android" && Platform.Version >= 33) {
        const granted = await PermissionsAndroid.request(
          PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS,
          {
            ...
            message: "This app needs notification permission to notify you.",
        ...
        console.log(
          "✅ No need to request notification permission (pre-Android 13 or iOS)"
        );
        // await registerPushToken();
      }
    } catch (error) {
      ...
      try {
        ...
        if (!responseData || responseData.length === 0) {
          ...
          if (newPage === 1) {
            ...
            setWorkingDomains(
              responseData.filter((el) => el.is_working === true)
            );
            setNotWorkingDomains(
              responseData.filter((el) => el.is_working !== true)
            );
          } else {
            setData((prevData) => {
              ...
              setWorkingDomains(
                updatedData.filter((el) => el.is_working === true)
              );
              setNotWorkingDomains(
                updatedData.filter((el) => el.is_working !== true)
              );
      ...
      <View style={styles.itemCard}>
        ...
        <View style={styles.detailsCard}>
          <View style={styles.statusCard}>
            {item?.status === "PAUSED" && (
              <Image style={styles.statusImage} source={images.pause} />
            )}
            {item?.status === "UP" && (
              <Text
                style={[
                  styles.statusText,
                  { color: item?.is_working ? "#22C55F" : "#BF232A" },
                ]}
              >
                UP
              </Text>
            ...
            <Text style={styles.availabilityText}>
              {"AVAILABILITY  "}
              <Text style={styles.availabilityValue}>{item?.availbility}</Text>
              {"  DOWNTIME  "}
              <Text style={styles.downtimeValue}>{item?.down_time}</Text>
            </Text>
          </View>
          <View style={{ flex: 1 }}>
        ...
        <View style={styles.checkCard}>
          <Text style={styles.intervalText}>{item?.interval_def}</Text>
        </View>
      </View>
    );
  }, []);
        ...
        <View style={styles.hearderCard}>
          <TouchableOpacity
            ...
            style={[
              ...
              { borderColor: borderColor, borderWidth: total ? 1 : 0 },
            ]}
          >
            <Text
              style={[
                FontsStyle.caption_text,
                { color: "#8F8F8F", marginTop: 10 },
              ]}
            >
              Total
            </Text>
            <Text
              style={[
                FontsStyle.heading_2_text,
                { fontWeight: "700", color: "#8F8F8F", marginBottom: 10 },
              ]}
            >
              {num?.total_monitors}
            </Text>
          ...
          <TouchableOpacity
            ...
            style={[
              styles.headerBtnStyle,
              { borderColor: borderColor, borderWidth: up && !down ? 1 : 0 },
            ]}
          >
            <Text
              style={[
            ...
            <Text
              style={[
                FontsStyle.heading_2_text,
                { fontWeight: "700", color: "#22C55F", marginBottom: 10 },
              ]}
            >
              {num?.up_monitors}
            </Text>
          ...
          >
            ...
            <Text
              style={[
                FontsStyle.heading_2_text,
                { fontWeight: "700", color: "#BF232A", marginBottom: 10 },
              ]}
            >
              {num?.down_monitors}
            </Text>
  ...
  return (
    <View style={styles.mainCard}>
      <FlatList
        ...
        ListFooterComponent={
          isLoading && <ActivityIndicator size="large" color="#04142E" />
        }
